...
export class ChucDanhCreateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getDonVi();
    if (!this.id) {
      this.modalInfo.Title = "Thêm mới chức danh";
    } else {
      if (this.isView == true)
        this.modalInfo.Title = "Chi tiết chức danh";
      else
        this.modalInfo.Title = "Chỉnh sửa chức danh";
  ...
  save(isContinue: boolean) {
    if (this.soTien) {
      this.model.luongCoBan = this.soTien.replace(/,/g, '');
    }
    if (!this.id) {
  ...
  create(isContinue: any) {
    this.chucDanhService.createChucDanh(this.model).subscribe(
      (data: any) => {
  ...
  update() {
    this.chucDanhService.updateChucDanh(this.id, this.model).subscribe(
      (data: any) => {
  ...
  clear() {
    ...
    var idPhongBan = this.model.idPhongBan;
    this.model = {
      idDonVi: idDonVi,
