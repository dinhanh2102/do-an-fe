...
import { LoaiCongCreateComponent } from '../loai-cong-create/loai-cong-create.component';
...
@Component({
  ...
  templateUrl: './loai-cong-manage.component.html',
  styleUrls: ['./loai-cong-manage.component.scss']
})
export class LoaiCongManageComponent implements OnInit {

  constructor(
    private messageService: MessageService,
    private modalService: NgbModal,
    private serviceLoaiCong: LoaiCongService,
  ...
  setToolbarConfig() {
    let meuOptions: MenuOptions = {
      ...
      searchOptions: {
        ...
        Placeholder: 'Tìm kiếm tên loại công',
        Items: [
        ]
      }
    };
  ...
  //Tìm kiếm loại công
  search() {
    this.serviceLoaiCong.searchLoaiCong(this.searchModel).subscribe((data: any) => {
      if (data.isStatus) {
  ...
  showConfirmDelete(Id: string) {
    this.messageService.showConfirm("Bạn có chắc muốn xoá loại công này không?").then(
      data => {
        this.delete(Id);
      }
    );
  ...
  //Xóa loại công
  delete(Id: string) {
    this.serviceLoaiCong.deleteLoaiCong(Id).subscribe(
      data => {
        if (data.isStatus) {
          this.messageService.showSuccess('Xóa loại công thành công!');
          this.search();
        }
      },
      error => {
  ...
  showCreateUpdate(id: string) {
    let activeModal = this.modalService.open(LoaiCongCreateComponent, { container: 'body', windowClass: 'loai-ca-create', backdrop: 'static' })
